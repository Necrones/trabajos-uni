...
echo "############################################################" 
echo "############################################################" > informePrioridadMayor.txt
echo "#                     Creative Commons                     #" >> informePrioridadMayor.txt
echo "#                                                          #" >> informePrioridadMayor.txt
echo "#                   BY - Atribución (BY)                   #" >> informePrioridadMayor.txt
echo "#                 NC - No uso Comercial (NC)               #" >> informePrioridadMayor.txt
echo "#                SA - Compartir Igual (SA)                 #" >> informePrioridadMayor.txt
echo "############################################################" >> informePrioridadMayor.txt
...
echo "#######################################################################" >> informePrioridadMayor.txt
echo "#                                                                     #" >> informePrioridadMayor.txt
echo "#                         INFORME DE PRÁCTICA                         #" >> informePrioridadMayor.txt
echo "#                         GESTIÓN DE PROCESOS                         #" >> informePrioridadMayor.txt
echo "#             -------------------------------------------             #" >> informePrioridadMayor.txt
echo "#     Nuevos alumnos:                                                 #" >> informePrioridadMayor.txt
echo "#     Alumnos: Omar Santos Bernabé				        #" >> informePrioridadMayor.txt
echo "#     Sistemas Operativos 2º Semestre                                 #" >> informePrioridadMayor.txt
echo "#     Grado en ingeniería informática (2015-2016)                     #" >> informePrioridadMayor.txt
echo "#                                                                     #" >> informePrioridadMayor.txt
echo "#######################################################################" >> informePrioridadMayor.txt
echo "" >> informePrioridadMayor.txt
		...
		for(( j=0 ; j<= ${#proceso[@]} ; j++ )){
			...
			if [ $contador -gt 1 ] #si el contador es mayor que uno
				then
					...
					echo " "
					exit; #Valor de la variable a 1 para un valor mal introducido					
			fi
		}
	}
...
#funcion que comprueba el numero de palabras introducidos
...
echo -e "\e[0;33m*				\e[1;36mAlgoritmo Prioridades(Mayor) \e[0m						\e[0;33m*"			
...
do
	...
	while [[ $npro != ?([+])+([0-9]) ]]; do
		echo echo -e "${red}ERROR: Introduzca un valor entero${NC}"
  		echo "Introduzca el numero de procesos a ejecutar : "
  		read npro 
	done
	while [ $npro = 0 ]; do
...
echo "" >> informeSJF.txt
	...
	echo "_________________________________________________________________________" >> prioridades.temp
		...
		if [ $opcion = "n" -o $opcion = "N" ] #si el usuario desea introducir los datos desde un fichero, es decir, datosPrederteminados.
			...
			echo Proceso: $nombre Tiempo de ejecución: $tiemp Prioridad del Proceso: $priorida   >> .datosEntrada.txt
			echo "" >> informePrioridadMayor.txt
			echo Proceso: $nombre Tiempo de ejecución: $tiemp Prioridad de Proceso: $priorida   >> informePrioridadMayor.txt
...
echo "" >> informePrioridadMayor.txt
echo -e "     Proceso        Ejecución        Prioridad         Espera        Respuesta   " >> informePrioridadMayor.txt	
...
for ((i=0;i<${#tiempo[@]};i++)){
	...
	if [ ${tiempo[$i]} -eq 0 ]    #si la posición 0 = 0 
	  then 
		...
		sleep "0.5"
		espera=`expr $respuesta`                     #variable que almacena el tiempo de espera de ese proceso, es decir, el tiempo de respuesta anterior
		respuesta=`expr $respuesta + ${tiempo[$i]}`  #variable que almacena la suma del tiempo de espera, mas el contenido del vector tiempo en esa posición
...
echo -e "*	${proceso[$i]}	*	${tiempo[$i]}	*	${prioridad[$i]}	*	$espera	*	$respuesta	* " >> informePrioridadMayor.txt
}
...
echo "" >> informePrioridadMayor.txt
echo -e "		* T.espera medio: $promedio_espera  -  * T.retorno medio: $promedio_respuesta " >> informePrioridadMayor.txt 
echo "" >> informePrioridadMayor.txt
...
if [ $datos = "s" ]
	then
	gedit informePrioridadMayor.txt
fi


#final de la ejecución del calculo de los tiempos de espera y respuesta de los procesos más los promedios
