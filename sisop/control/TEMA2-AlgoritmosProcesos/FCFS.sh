...
echo "#             -------------------------------------------             #" >> informeFCFS.txt
echo "#     Nuevos alumnos:                                                 #" >> informeFCFS.txt
echo "#     Alumnos: Omar Santos Bernabé				        #" >> informeFCFS.txt
echo "#     Sistemas Operativos 2º Semestre                                 #" >> informeFCFS.txt
echo "#     Grado en ingeniería informática (2015-2016)                     #" >> informeFCFS.txt
echo "#                                                                     #" >> informeFCFS.txt
echo "#######################################################################" >> informeFCFS.txt
echo "" >> informeFCFS.txt
...
echo -e "\e[0;33m*				\e[1;36mAlgoritmo FCFS \e[0m						\e[0;33m*"			
...
echo "" >> informeFCFS.txt
	echo "" > fcfs.temp
	echo "| 	Proceso		|	T. Llegada	 |	 T.Ejecución	 |" >> fcfs.temp
	echo "_________________________________________________________________________" >> fcfs.temp
	  ...
	  then
		...
		if [ $p == 1 ]
			then
				...
				echo Proceso ${proceso[$p]} Llegada ${llegada[$p]} Ejecucion ${tiempo[$p]}   >> ./informeFCFS.txt
		...
		echo -n " 	 $tiemp 		 |"  >> fcfs.temp
		clear
		cat fcfs.temp

		echo ""
		echo Proceso $nombre Llegada $llegad Ejecucion $tiemp   >> informeFCFS.txt
		echo Proceso $nombre Llegada $llegad Ejecucion $tiemp   >> ejemplo.txt
		echo "" >> fcfs.temp
		echo "__________________________________________________________________________" >> fcfs.temp
		echo "" >> fcfs.temp	
...
echo -e "     Proceso         Llegada        Ejecución         Espera        Respuesta   " >> ./informeFCFS.txt


if [ $opcion == 'R' ] || [ $opcion == 'r' ]
then
for ((i=1;i<=${#tiempo[@]};i++)){
	if [ ${tiempo[$i]} -eq 0 ]    #Si la posición 0 entonces T.ejecucion = 0 
	  then 
...
echo -e " *\t${proceso[$i]}\t*\t${llegada[$i]}\t*\t${tiempo[$i]}\t*\t$espera\t*\t$respuesta\t   " >> ./informeFCFS.txt
echo ""
}
else 
for ((i=1;i<=${#tiempo[@]};i++)){
...
echo -e " * T.espera medio: $promedio_espera  -  * T.retorno medio: $promedio_respuesta  " >> ./informeFCFS.txt
